...
from .models import BrokerEnvelope, SyscallEvent
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;font-family:"IBM Plex Sans","Segoe UI",sans-serif;color:var(--ink);background:radial-gradient(circle at top left,rgba(15,118,110,.18),transparent 36%),radial-gradient(circle at top right,rgba(245,158,11,.16),transparent 28%),linear-gradient(180deg,#f7f3eb 0%,#efe7da 100%);min-height:100vh}
    header{padding:24px 28px 8px}h1{margin:0;font-size:1.8rem;letter-spacing:-.04em}.subtitle{color:var(--muted);margin-top:8px;max-width:760px;line-height:1.5}
    ...
    .panel-header{display:flex;align-items:center;justify-content:space-between;padding:18px 20px 10px}.panel-title{font-weight:700;font-size:1rem}.panel-body{padding:0 18px 18px}.badge{font-size:.78rem;color:white;background:var(--accent);border-radius:999px;padding:6px 10px}
    .queue-list,.log-list{display:grid;gap:12px;max-height:calc(100vh - 200px);overflow:auto;padding-right:4px}.event{border:1px solid var(--line);border-radius:18px;padding:14px;background:rgba(255,255,255,.8)}.event.focused{outline:2px solid rgba(15,118,110,.24)}
    .event-head{display:flex;align-items:start;justify-content:space-between;gap:12px;margin-bottom:8px}.syscall{font-family:"IBM Plex Mono","Consolas",monospace;font-size:.85rem;color:var(--accent)}.summary{font-weight:650;line-height:1.45;word-break:break-word}
    ...
    .status{font-size:.78rem;border-radius:999px;padding:5px 9px;background:rgba(31,42,48,.08);color:var(--ink);white-space:nowrap}.status.allowed{background:rgba(15,118,110,.16);color:#0b5f59}.status.denied,.status.error{background:rgba(185,28,28,.16);color:#991b1b}.status.timeout{background:rgba(245,158,11,.18);color:#92400e}
    ...
    @media (max-width:1024px){main{grid-template-columns:1fr}.queue-list,.log-list{max-height:none}}
  </style>
</head>
<body>
  <header>
    <h1>gVisor Syscall Approval Console</h1>
    <div class="subtitle">Open Interpreter CLI stays in the terminal. This page only shows hooked syscalls and lets you approve with <span class="keycap">y</span> or deny with <span class="keycap">n</span>.</div>
    <div class="banner"><strong>Web page:</strong> syscall approvals only.<br><strong>Terminal:</strong> actual agent conversation and execution output.</div>
  </header>
  <main>
    <section class="panel">
  ...
  <script>
    const pendingList=document.getElementById("pending-list"); const eventLog=document.getElementById("event-log"); const pendingCount=document.getElementById("pending-count"); const connectionState=document.getElementById("connection-state");
    const state={events:[],focusedId:null};
    ...
    function applyEnvelope(envelope){
      ...
      if(envelope.type==="snapshot"){state.events=payload.events;}
...
class ApprovalBroker:
    ...
    async def start(self) -> None:
        ...
        self._ipc_server = await asyncio.start_unix_server(self._handle_ipc_client, path=str(self.socket_path))
    ...
    async def snapshot(self) -> dict[str, Any]:
        ...
        return BrokerEnvelope("snapshot", {"events": events}).to_dict()
...
async def create_app(socket_path: Path, decision_timeout: float) -> web.Application:
    broker = ApprovalBroker(socket_path=socket_path, decision_timeout=decision_timeout)
    await broker.start()
    app = web.Application()
    app["broker"] = broker
